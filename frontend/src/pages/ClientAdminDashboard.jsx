...
import { Building2, Users, Database, Sparkles, TrendingUp, CheckCircle, Code, ShieldCheck, Activity, Plus, RefreshCw, Terminal, PhoneCall, MessageSquare } from 'lucide-react';
...
export default function ClientAdminDashboard() {
  ...
  const [selectedRecommendedModel, setSelectedRecommendedModel] = useState('DeepSeek V3');
  ...
  const planCatalog = [
    { id: 'bulk_ivr', name: '📞 Bulk IVR Calling', category: 'Telecom & IVR', stt: 0.0005, tts: 0.0005, telecom: 0.007, llm: 0.0001, costPerMinute: 0.0081, desc: 'High-concurrency outbound DTMF & voice broadcast via Twilio Trunking.' },
    ...
    { id: 'call_recording', name: '⏺️ Call Recording', category: 'Telecom & IVR', stt: 0.0, tts: 0.0, telecom: 0.003, llm: 0.0, costPerMinute: 0.0030, desc: 'Encrypted multi-channel stereo WAV/MP3 recording on AWS S3.' },
    ...
    { id: 'lead_tag_filter', name: '🏷️ Tag, Filter & Transfer Leads', category: 'Analytics & CRM', stt: 0.0, tts: 0.0, telecom: 0.001, llm: 0.0005, costPerMinute: 0.0015, desc: 'Automated intent tagging, hot lead filtering and routing.' },
    ...
    { id: 'admin_team_management', name: '🛡️ Admin & Team Member Management', category: 'Analytics & CRM', stt: 0.0, tts: 0.0, telecom: 0.0005, llm: 0.0005, costPerMinute: 0.0010, desc: 'Role-based access controls (RBAC) and agent productivity scores.' },
    ...
    { id: 'auto_email_meeting', name: '✉️ Auto Email on Meeting Booked', category: 'Productivity & Sync', stt: 0.0, tts: 0.0, telecom: 0.001, llm: 0.0005, costPerMinute: 0.0015, desc: 'Instant calendar invite and confirmation email dispatch via SendGrid.' },
    ...
    { id: 'male_female_voice', name: '🎙️ Male / Female / Neural Voice', category: 'AI Voice Calling Bot', stt: 0.0, tts: 0.0150, telecom: 0.002, llm: 0.0005, costPerMinute: 0.0175, desc: 'Choice of 50+ hyper-realistic ElevenLabs and Murf neural voices.' },
  ...
  const allModelsCatalog = [
    { id: 'gpt-4o', name: 'GPT-4o', provider: 'OpenAI', input_cost: 0.005, output_cost: 0.015, latency: 450, context: '128k', strength: 'Universal Intelligence & Multimodal Vision', badge: 'Flagship' },
    { id: 'claude-3-5-sonnet', name: 'Claude 3.5 Sonnet', provider: 'Anthropic', input_cost: 0.003, output_cost: 0.015, latency: 420, context: '200k', strength: 'Complex Coding & Nuanced Human-Like Writing', badge: 'Top Accuracy' },
    { id: 'claude-3-haiku', name: 'Claude 3 Haiku', provider: 'Anthropic', input_cost: 0.00025, output_cost: 0.00125, latency: 210, context: '200k', strength: 'Sub-250ms Acoustic Speed & Voice AI Calling Bots', badge: 'Recommended Voice' },
    { id: 'llama-3-70b', name: 'Llama 3 70B', provider: 'Meta / OpenRouter', input_cost: 0.0007, output_cost: 0.0008, latency: 310, context: '8k', strength: 'Best Open-Source Performance & Data Pipelines', badge: 'Recommended Code' },
    { id: 'llama-3-8b', name: 'Llama 3 8B', provider: 'Meta / OpenRouter', input_cost: 0.0001, output_cost: 0.0001, latency: 180, context: '8k', strength: 'Ultra-Cheapest High-Speed Edge Bot Queries', badge: 'Cheapest Overall' },
    { id: 'gemini-1-5-pro', name: 'Gemini 1.5 Pro', provider: 'Google', input_cost: 0.0035, output_cost: 0.0105, latency: 520, context: '2,000k', strength: 'Massive 2 Million Token Context Window & Document RAG', badge: 'Best Long-Context' },
    { id: 'gemini-1-5-flash', name: 'Gemini 1.5 Flash', provider: 'Google', input_cost: 0.00035, output_cost: 0.00105, latency: 230, context: '1,000k', strength: 'High Speed Multimodal Processing & Fast Summaries', badge: 'Recommended RAG' },
    { id: 'mistral-large', name: 'Mistral Large 2', provider: 'Mistral AI', input_cost: 0.002, output_cost: 0.006, latency: 380, context: '128k', strength: 'Top European Multilingual & Logical Reasoning', badge: 'Multilingual' },
    { id: 'deepseek-v3', name: 'DeepSeek V3', provider: 'DeepSeek', input_cost: 0.00014, output_cost: 0.00028, latency: 290, context: '64k', strength: 'Ultra-Low Cost Tier-1 Math, Programming & Logic', badge: 'Recommended Overall' },
    { id: 'cohere-command-r-plus', name: 'Command R+', provider: 'Cohere', input_cost: 0.003, output_cost: 0.015, latency: 490, context: '128k', strength: 'Enterprise RAG & Autonomous Tool Calling', badge: 'Enterprise RAG' }
  ];
  ...
  return (
    <div>
      <div className="flex-between mb-2">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ color: 'white', fontWeight: 600 }}>{currentOrg.company_name || 'Acme Voice & Chatbot Corp'}</span>
          </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem', flexWrap: 'wrap' }}>
        ...
        <button 
          className={`btn ${activeTab === 'telemetry' ? '' : 'btn-secondary'}`}
          onClick={() => setActiveTab('telemetry')}
          style={{ padding: '0.6rem 1.25rem' }}
        >
          <Sparkles size={18} /> Automated Telemetry & Smart Router
        </button>
        <button 
          ...
          style={{ padding: '0.6rem 1.25rem' }}
        >
          <Code size={18} /> Platform Integrations (Calling Agents & Chatbots)
        </button>
        <button 
          ...
          style={{ padding: '0.6rem 1.25rem' }}
        >
          <Users size={18} /> Workforce & Agent Accounts ({employees.length})
        </button>
      </div>
      ...
      {activeTab === 'comparison' && (
        <div>
          ...
          <div className="card" style={{ 
            ...
            background: 'linear-gradient(135deg, rgba(30, 33, 48, 0.95), rgba(249, 115, 22, 0.12))',
            boxShadow: '0 8px 30px rgba(0,0,0,0.4)'
          }}>
            <div className="flex-between mb-2">
              <div>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'white', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <Sparkles color="#f97316" /> Enterprise LLM Model Comparison & Token Calculator
                </h2>
            ...
            <div style={{ background: 'rgba(0,0,0,0.4)', padding: '1.5rem', borderRadius: '14px', border: '1px solid rgba(249, 115, 22, 0.45)', marginBottom: '1.5rem', background: 'linear-gradient(135deg, rgba(30, 33, 48, 0.95), rgba(249, 115, 22, 0.15))' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <h3 style={{ margin: 0, color: 'white', fontSize: '1.25rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px', marginBottom: '1rem' }}>
                <div style={{ gridColumn: '1 / -1' }}>
                  <select 
                    ...
                    style={{ width: '100%', fontSize: '1.05rem', fontWeight: 700, padding: '0.75rem 1rem', background: '#1e2130', color: 'white', border: '2px solid #f97316', borderRadius: '10px', cursor: 'pointer' }}
                  >
                    {bundleTiers.map(b => (
                      <option key={b.id} value={b.id} style={{ background: '#131520', color: 'white' }}>
                        {b.name} — {b.badge}
                      </option>
                    ))}
                    <option value="custom" style={{ background: '#131520', color: '#fdba74' }}>
                      🛠️ CUSTOM BUNDLE — Hand-Picked ({selectedPlans.length} Capabilities Selected)
                    </option>
                  </select>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '10px' }}>
                {bundleTiers.map(b => {
                  ...
                  return (
                    ...
                    >
                      {isCurrent && (
                        <div style={{ position: 'absolute', top: '-10px', right: '10px', background: b.color, color: 'white', fontSize: '0.65rem', fontWeight: 800, padding: '2px 8px', borderRadius: '10px' }}>
                          ✓ ACTIVE TIER
                        </div>
                      )}
                      <div style={{ fontWeight: 800, color: isCurrent ? 'white' : b.color, fontSize: '0.95rem', marginBottom: '4px' }}>
                        {b.name.split(' ')[0]} {b.name.split(' ')[1]}
                      </div>
                      <div style={{ fontSize: '0.75rem', color: '#a5b4fc', fontWeight: 600, marginBottom: '6px' }}>
                        {b.badge.split(' • ')[1]}
                      </div>
            ...
            <div style={{ background: 'rgba(0,0,0,0.4)', padding: '1.5rem', borderRadius: '14px', border: '1px solid var(--border-color)', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <h3 style={{ margin: 0, color: 'white', fontSize: '1.15rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    🎛️ Step 2: Customize / Fine-Tune Capabilities in Selected Plan <span className="badge primary" style={{ fontSize: '0.75rem' }}>{selectedPlans.length} / 20 Selected</span>
                  </h3>
              ...
              {showPlanDropdown && (
                <div style={{ background: '#131520', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--primary)', marginBottom: '1.25rem', maxHeight: '420px', overflowY: 'auto' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.75rem' }}>
                    <span style={{ fontWeight: 700, color: '#a5b4fc', fontSize: '0.9rem' }}>Check / Uncheck Individual Capabilities (`Exact 20 from PDF`):</span>
                      ...
                      return (
                        <div 
                          ...
                          style={{ 
                            ...
                            borderRadius: '8px', 
                            cursor: 'pointer',
                            background: isSelected ? 'rgba(249, 115, 22, 0.22)' : 'rgba(255,255,255,0.03)',
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '0.85rem', fontWeight: isSelected ? 700 : 500, color: isSelected ? 'white' : '#cbd5e1', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {plan.name}
                            </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '1.25rem', maxHeight: showPlanDropdown ? '100px' : 'none', overflowY: 'auto' }}>
                {planCatalog.filter(p => selectedPlans.includes(p.id)).map(plan => (
                  ...
                  >
                    <span>{plan.name}</span>
                    <span style={{ color: '#fca5a5', fontWeight: 800 }}>×</span>
                  </div>
                ))}
              </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem', marginTop: '0.5rem' }}>
                    <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#fdba74', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
                      ...
                      {showCallingSliders && (
                        <>
                          <div style={{ background: 'rgba(249, 115, 22, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(249, 115, 22, 0.28)' }}>
                            <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#fdba74', fontWeight: 600, marginBottom: '6px' }}>
                              <span>📞 Monthly Telecom / IVR Call Sessions:</span>
                              <span style={{ color: 'white', fontWeight: 700 }}>{monthlyInteractions.toLocaleString()} Sessions</span>
                            </label>
                            <input 
                          ...
                          <div style={{ background: 'rgba(249, 115, 22, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(249, 115, 22, 0.28)' }}>
                            <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#ffffff', fontWeight: 600, marginBottom: '6px' }}>
                              <span>⏱️ Average Call / Session Duration:</span>
                              <span style={{ color: 'white', fontWeight: 700 }}>{avgDurationMinutes.toFixed(1)} Minutes</span>
                            </label>
                            <input 
                      ...
                      {showWhatsappSliders && (
                        <>
                          <div style={{ background: 'rgba(16, 185, 129, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(16, 185, 129, 0.25)' }}>
                            <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#34d399', fontWeight: 600, marginBottom: '6px' }}>
                              <span>💬 Monthly WhatsApp Messages / Alerts:</span>
                              <span style={{ color: 'white', fontWeight: 700 }}>{whatsappMonthlyMessages.toLocaleString()} Messages</span>
                            </label>
                            <input 
                          ...
                          <div style={{ background: 'rgba(16, 185, 129, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(16, 185, 129, 0.25)' }}>
                            <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#6ee7b7', fontWeight: 600, marginBottom: '6px' }}>
                              <span>🔄 Avg Turns / Messages per WhatsApp Chat:</span>
                              <span style={{ color: 'white', fontWeight: 700 }}>{whatsappAvgTurns} Turns</span>
                            </label>
                            <input 
                      ...
                      {showCrmSliders && (
                        <div style={{ background: 'rgba(245, 158, 11, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(245, 158, 11, 0.25)' }}>
                          <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#fbbf24', fontWeight: 600, marginBottom: '6px' }}>
                            <span>📈 Monthly CRM Reports & Lead Routing Triggers:</span>
                            <span style={{ color: 'white', fontWeight: 700 }}>{crmMonthlyEvents.toLocaleString()} Events</span>
                          </label>
                          <input 
                      ...
                      {showAiSliders && (
                        <div style={{ background: 'rgba(236, 72, 153, 0.08)', padding: '1rem', borderRadius: '10px', border: '1px solid rgba(236, 72, 153, 0.25)' }}>
                          <label className="label" style={{ display: 'flex', justifyContent: 'space-between', color: '#f472b6', fontWeight: 600, marginBottom: '6px' }}>
                            <span>🤖 Autonomous AI Voice & Meeting Sessions:</span>
                            <span style={{ color: 'white', fontWeight: 700 }}>{aiAutonomousSessions.toLocaleString()} AI Sessions</span>
                          </label>
                          <input 
          ...
          {(() => {
            ...
            const sttRate = hasVoice || hasAiVoice ? 0.0043 : (hasTelecom ? 0.0018 : 0); // Deepgram Nova-2 ($0.0043/min)
            const ttsRate = hasNeuralVoice || hasAiVoice ? 0.0150 : (hasVoice ? 0.0060 : 0); // ElevenLabs Turbo / Murf ($0.0150/min)
            const telecomRate = hasTelecom || hasVoice || hasAiVoice ? 0.0070 : 0; // Twilio Trunking ($0.0070/min)
            ...
            const whatsappTotal = hasWhatsApp ? whatsappMonthlyMessages * whatsappRate : 0;
            const crmTotal = hasCrm ? crmMonthlyEvents * 0.0015 : 0;

            // Capped unified LLM Gateway intelligence rate across active options
            const llmTotal = (hasAiVoice ? aiAutonomousSessions * avgDurationMinutes * 0.0030 : 0) + (hasWhatsApp ? whatsappMonthlyMessages * 0.0015 : 0) + (hasCrm ? crmMonthlyEvents * 0.0005 : 0) + (hasTelecom ? monthlyInteractions * 0.0010 : 0);
            ...
            const competitorMonthlyTotal = Math.max(monthlyTotal * 3.4, callingMinutes * 0.085 + (whatsappMonthlyMessages * 0.012) + (crmMonthlyEvents * 0.005));
            const savingsPercent = Math.round(((competitorMonthlyTotal - monthlyTotal) / (competitorMonthlyTotal || 1)) * 100);

            return (
              <div style={{ 
                background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.08), rgba(249, 115, 22, 0.16))', 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', alignItems: 'center' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#6ee7b7', fontWeight: 800, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '8px' }}>
                      <CheckCircle size={18} /> Direct Vendor Infrastructure Pricing Engine (`Zero Agency Markup`)
                    </div>
                    <h3 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'white', margin: '0 0 10px 0' }}>
                      {activePlans.length} / 20 Capabilities Activated
                    </h3>
                    <p style={{ margin: 0, fontSize: '0.92rem', color: '#d1fae5', lineHeight: '1.6' }}>
                      By utilizing our Gateway directly with **ElevenLabs Neural TTS**, **Murf AI**, **Deepgram Nova-2 STT**, and **Twilio SIP Trunking**, your selected capabilities are auto-optimized for sub-300ms acoustic SLA without double-charging wrapper fees.
                    </p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px', marginTop: '1.25rem' }}>
                      {(hasVoice || hasTelecom || hasAiVoice) && (
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Deepgram STT (Speech-to-Text)</div>
                          <div style={{ color: '#6ee7b7', fontWeight: 700 }}>~${sttTotal.toFixed(2)} / mo</div>
                        </div>
                      )}
                      {(hasNeuralVoice || hasVoice || hasAiVoice) && (
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>ElevenLabs / Murf TTS (Voice)</div>
                          <div style={{ color: '#f472b6', fontWeight: 700 }}>~${ttsTotal.toFixed(2)} / mo</div>
                        </div>
                      )}
                      {(hasTelecom || hasVoice || hasAiVoice) && (
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Twilio SIP Trunking & Telecom</div>
                          <div style={{ color: '#a5b4fc', fontWeight: 700 }}>~${telecomTotal.toFixed(2)} / mo</div>
                        </div>
                      )}
                      {hasWhatsApp && (
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Meta / WhatsApp Cloud API</div>
                          <div style={{ color: '#34d399', fontWeight: 700 }}>~${whatsappTotal.toFixed(2)} / mo</div>
                        </div>
                      )}
                      {hasCrm && (
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>CRM Reports & Webhook Events</div>
                          <div style={{ color: '#fbbf24', fontWeight: 700 }}>~${crmTotal.toFixed(2)} / mo</div>
                        </div>
                      )}
                      <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>LLM Intelligence (Claude / DeepSeek)</div>
                        <div style={{ color: '#fde047', fontWeight: 700 }}>~${llmTotal.toFixed(2)} / mo</div>
                      </div>
                    </div>
                  </div>

                  <div style={{ background: 'rgba(0,0,0,0.4)', padding: '1.5rem', borderRadius: '14px', border: '1px solid rgba(16, 185, 129, 0.4)', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '14px' }}>
                      Effective rate across {activePlans.length} features: <strong style={{ color: 'white' }}>${totalUnitRate.toFixed(4)} / min</strong>
                    </div>
                    <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px dashed rgba(239, 68, 68, 0.3)', padding: '0.75rem', borderRadius: '8px', marginBottom: '14px' }}>
                      <div style={{ fontSize: '0.75rem', color: '#fca5a5' }}>Competitor Agency Wrappers (Legacy Rate)</div>
                      <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#f87171', textDecoration: 'line-through' }}>
          ...
          {(() => {
            ...
            const sttVendors = [
              { name: 'Deepgram Nova-2', provider: 'Deepgram', rate: 0.0043, latency: 190, accuracy: '99.4% (Word-level timestamps)', badge: 'Recommended Gateway Choice', isTop: true },
              { name: 'AssemblyAI Universal-1', provider: 'AssemblyAI', rate: 0.0065, latency: 320, accuracy: '98.9% (High noise robustness)', isTop: false },
              { name: 'OpenAI Whisper API', provider: 'OpenAI', rate: 0.0060, latency: 650, accuracy: '98.2% (Batch/File optimized)', isTop: false },
              { name: 'Google Cloud STT v2', provider: 'Google Cloud', rate: 0.0160, latency: 450, accuracy: '96.5% (Standard telco model)', isTop: false }
            ];

            const ttsVendors = [
              { name: 'ElevenLabs Turbo v2.5', provider: 'ElevenLabs', rate: 0.0150, latency: 180, quality: '50+ Ultra-realistic neural clones & emotive inflection', badge: 'Recommended Gateway Choice', isTop: true },
              { name: 'Murf AI Falcon HD', provider: 'Murf AI', rate: 0.0120, latency: 220, quality: 'Excellence in multi-lingual Indian/Hinglish voices', badge: 'Best Regional Choice', isTop: false },
              { name: 'PlayHT 2.0 Turbo', provider: 'PlayHT', rate: 0.0180, latency: 240, quality: 'High conversational speed & custom cloning', isTop: false },
              { name: 'OpenAI TTS HD', provider: 'OpenAI', rate: 0.0300, latency: 450, quality: 'Standard robotic neural synthesis', isTop: false }
            ];

            const telecomVendors = [
              { name: 'Twilio Elastic SIP Trunking', provider: 'Twilio Cloud', rate: 0.0070, uptime: '99.999% Tier-1 Global Carrier SLA', badge: 'Recommended Gateway Choice', isTop: true },
              { name: 'Voximplant Cloud SIP', provider: 'Voximplant', rate: 0.0060, uptime: '99.99% Sub-100ms RTP acoustic bridge', isTop: false },
              { name: 'Plivo SIP Trunking', provider: 'Plivo', rate: 0.0065, uptime: '99.95% Standard regional routing', isTop: false },
              { name: 'Legacy Telco PRI / Agency Wrappers', provider: 'Traditional Agencies', rate: 0.0550, uptime: 'Per-channel monthly licensing + markup', badge: 'Overpriced Wrapper', isTop: false }
            ];

            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {/* 🎙️ Dynamic STT Comparison Grid (When Voice/Speech/IVR is selected) */}
                {(hasVoice || hasTelecom) && (
                  <div className="card">
                    <div className="flex-between mb-2">
                      <div>
                        <h3 style={{ fontSize: '1.35rem', fontWeight: 700, color: '#ffffff', margin: 0 }}>
                          🎙️ Speech-to-Text (STT) Acoustic Vendor Comparison Grid (`Activated for Voice/IVR Suite`)
                        </h3>
                    ...
                    <div className="table-container">
                      <table className="data-table">
                        ...
                        <tbody>
                          {sttVendors.map(v => (
                            <tr key={v.name} style={{ background: v.isTop ? 'rgba(255, 255, 255, 0.08)' : 'transparent', borderLeft: v.isTop ? '3px solid #ffffff' : 'none' }}>
                              <td>
                              ...
                              <td style={{ fontSize: '0.85rem', color: '#e2e8f0' }}>{v.accuracy}</td>
                              <td style={{ fontWeight: 600, color: '#fdba74' }}>${v.rate.toFixed(4)} / min</td>
                              <td style={{ fontWeight: 700, color: 'white' }}>${(monthlyInteractions * avgDurationMinutes * v.rate).toFixed(2)} / mo</td>
                              <td>
                                {v.isTop ? (
                        ...
                        <tbody>
                          {ttsVendors.map(v => (
                            <tr key={v.name} style={{ background: v.isTop ? 'rgba(255, 255, 255, 0.08)' : 'transparent', borderLeft: v.isTop ? '3px solid #ffffff' : 'none' }}>
                              ...
                              <td style={{ fontSize: '0.85rem', color: '#e2e8f0' }}>{v.quality}</td>
                              <td style={{ fontWeight: 600, color: '#fdba74' }}>${v.rate.toFixed(4)} / min</td>
                              <td style={{ fontWeight: 700, color: 'white' }}>${(monthlyInteractions * avgDurationMinutes * v.rate).toFixed(2)} / mo</td>
                              <td>
                                {v.isTop ? (
                      ...
                      <table className="data-table">
                        ...
                        <tbody>
                          {telecomVendors.map(v => (
                            <tr key={v.name} style={{ background: v.isTop ? 'rgba(249, 115, 22, 0.12)' : 'transparent', borderLeft: v.isTop ? '3px solid #f97316' : 'none' }}>
                              <td>
                                <div style={{ fontWeight: 700, color: 'white' }}>{v.name}</div>
                                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{v.provider}</div>
                              </td>
                              <td style={{ fontSize: '0.85rem', color: '#e2e8f0' }}>{v.uptime}</td>
                ...
                <div className="card">
                  <div className="flex-between mb-2">
                    <div>
                      <h3 style={{ fontSize: '1.35rem', fontWeight: 700, color: 'white', margin: 0 }}>
                  ...
                  <div className="table-container">
                    <table className="data-table">
                      ...
                      <tbody>
                        {allModelsCatalog.map(m => {
                          const singleCost = ((compInputTokens / 1000) * m.input_cost) + ((compOutputTokens / 1000) * m.output_cost);
                          const bulkCost = singleCost * monthlyInteractions;
                          const isTop = m.name === selectedRecommendedModel;

                          return (
                            <tr key={m.id} style={{ background: isTop ? 'rgba(16, 185, 129, 0.08)' : 'transparent', borderLeft: isTop ? '3px solid #10b981' : 'none' }}>
                              <td>
                                <div style={{ fontWeight: 700, color: 'white', fontSize: '1.02rem' }}>{m.name}</div>
                                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{m.provider}</div>
                                {m.badge && (
                                  <span style={{ display: 'inline-block', marginTop: '4px', fontSize: '0.7rem', background: 'rgba(255, 255, 255, 0.18)', color: '#ffffff', padding: '2px 6px', borderRadius: '4px' }}>
                                    {m.badge}
                                  </span>
                                )}
                              </td>
                              <td style={{ fontSize: '0.85rem', color: '#e2e8f0', maxWidth: '220px' }}>{m.strength}</td>
                              <td style={{ fontWeight: 600, color: '#fdba74' }}>{m.context}</td>
                              <td style={{ color: 'var(--text-muted)' }}>${m.input_cost.toFixed(5)}</td>
                              <td style={{ color: 'var(--text-muted)' }}>${m.output_cost.toFixed(5)}</td>
                              <td style={{ fontWeight: 700, color: 'white' }}>${singleCost.toFixed(4)}</td>
      ...
      {activeTab === 'telemetry' && (
        <div>
          <div className="card" style={{ marginBottom: '2rem', border: '1px solid rgba(255, 255, 255, 0.2)', background: 'linear-gradient(135deg, rgba(30, 33, 48, 0.9), rgba(249, 115, 22, 0.12))' }}>
            <div className="flex-between">
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.35rem', fontWeight: 800, color: 'white', marginBottom: '6px' }}>
                  <Sparkles color="#f97316" /> AI Gateway Automated Traffic Analyzer
                </div>
              ...
              <div style={{ textAlign: 'right', background: 'rgba(16, 185, 129, 0.1)', padding: '1rem 1.5rem', borderRadius: '16px', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                <div style={{ fontSize: '0.85rem', color: '#6ee7b7', fontWeight: 600 }}>Estimated Cost Reduction</div>
            ...
            {telemetry && telemetry.traffic_breakdown && (
              <div style={{ marginTop: '2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: '1.5rem' }}>
                {telemetry.traffic_breakdown.map((item, idx) => (
                  <div key={idx} style={{ background: 'rgba(0,0,0,0.3)', padding: '1.5rem', borderRadius: '14px', border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {item.integration_type.toLowerCase().includes('voice') ? (
                          <div style={{ padding: '10px', background: 'rgba(249, 115, 22, 0.18)', color: '#f97316', borderRadius: '10px' }}><PhoneCall size={20} /></div>
                        ) : (
                          <div style={{ padding: '10px', background: 'rgba(255, 255, 255, 0.18)', color: '#ffffff', borderRadius: '10px' }}><MessageSquare size={20} /></div>
                        )}
                        <div>
                          <h4 style={{ color: 'white', fontSize: '1.1rem', fontWeight: 700 }}>{item.integration_type}</h4>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '10px', marginBottom: '1rem' }}>
                      <div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Current Model Used</div>
                        <div style={{ fontWeight: 600, color: '#fca5a5' }}>{item.current_model}</div>
                      </div>
                    ...
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', lineHeight: '1.5', margin: 0 }}>
                      <strong style={{ color: 'white' }}>Analysis:</strong> {item.reason}
                    </p>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'integrations' && (
        <div className="card">
          <h3 style={{ fontSize: '1.4rem', fontWeight: 700, color: 'white', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <label className="label" style={{ color: 'white', fontWeight: 600 }}>Your Organization Gateway API Key</label>
            <div style={{ display: 'flex', gap: '10px' }}>
              <input 
                type="text" className="input" readOnly 
                value={`bj_live_gateway_${currentOrg._id || '9a8b7c6d5e4f3a2b'}_secret`} 
                style={{ fontFamily: 'monospace', background: 'rgba(0,0,0,0.4)', color: '#fdba74' }}
              />
              <button className="btn" onClick={() => alert('API Key copied to clipboard!')}>Copy Key</button>
            </div>
          </div>

          <h4 style={{ color: 'white', marginBottom: '1rem' }}>1. Python / LangChain / OpenAI SDK Drop-in Replacement</h4>
          <pre style={{ background: 'rgba(0,0,0,0.4)', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border-color)', color: '#e2e8f0', overflowX: 'auto', fontSize: '0.9rem', marginBottom: '2rem', fontFamily: 'monospace' }}>
...
print(response.choices[0].message.content)`}
          ...
          <h4 style={{ color: 'white', marginBottom: '1rem' }}>2. Node.js / ElevenLabs Voice AI Agent Webhook</h4>
          <pre style={{ background: 'rgba(0,0,0,0.4)', padding: '1.25rem', borderRadius: '12px', border: '1px solid var(--border-color)', color: '#e2e8f0', overflowX: 'auto', fontSize: '0.9rem', fontFamily: 'monospace' }}>
{`// Configure your Voice Calling Bot (ElevenLabs / Twilio / Vapi) to route through Binjwa
            ...
            <table className="data-table">
              ...
              <tbody>
                {employees.length === 0 ? (
                  ...
                  employees.map(emp => (
                    <tr key={emp._id}>
                      <td style={{ fontWeight: 600, color: 'white' }}>{emp.name}</td>
      ...
      {showEmpModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '100%', maxWidth: '480px', padding: '2rem' }}>
            <h3 style={{ fontSize: '1.4rem', marginBottom: '1.5rem', color: 'white' }}>Create Employee / Agent Account</h3>
